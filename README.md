...
account_pool.py                          # 账号轮询 / 冷却踢出 / inflight 负载分散
conversation_affinity.py                 # 会话粘性 / prompt_cache_key / response 链
model_health.py                          # 模型探测 / free-usage 识别
settings_store.py                        # 系统设置热更新
sub2api_client.py                        # sub2api 登录 / 推送 / 账号容量
history_compact.py                       # 大上下文压缩 / 出站工具 gap
proxy_pool.py / quota.py                 # 出站代理池 / 硬配额踢出
task_log.py                              # 任务日志入口

# 存储
store/                                   # Redis + PostgreSQL 适配
store/pool_redis.py                      # RR 游标 / cooldown / inflight
store/affinity_redis.py                  # 会话粘性 Redis
migrate_json_to_pg.py                    # JSON → PG

# 脚本（见 scripts/README.md）
scripts/build_admin_assets.py            # 管理台静态资源打包
scripts/upgrade_from_file_backend.sh     # file → hybrid 升级
scripts/_test_*.py                       # 本地回归（粘性 / 冷却 / 踢出 / 终态帧）
